...
namespace SpleefResurgence
{
    public class SpleefGame
    {
        ...
        private void StartRound(CommandArgs args, string[] GameType, Map GameArena, int GimmickAmount)
        {
            ...
            GiveEveryoneArmor(CurrentMusicBoxID, Players: AlivePlayers.FindAll(p => spleefSettings.GetSettings(p.accName).GetMusicBox));
                ...
                foreach (Playering player in PlayerInfo)
                {
                    if (player.isIngame)
                    {
                        ...
                        if (plr == plrOG && player.isAlive)
                        {
                            ...
                            inventoryEdit.AddItem(plr, 8, 1, ItemID.ScryingOrb);
